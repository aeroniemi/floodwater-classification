...
def apply_mask_img(mask, fx=None, fy=None):
    # print(mask.shape, fx.shape)
    data = fx or fy
    output = []
    for img in range(mask.shape[0]):
        valid = np.delete(fx[img, :, :], mask[img, :], axis=0)
        output.append(valid)
    return np.ndarray(output, dtype="object")


def apply_mask_flattened(mask, fx=None, fy=None):
    mask = np.ravel(mask)
    fx = np.reshape(fx, (fx.shape[0] * fx.shape[1], fx.shape[2]))
    return np.delete(fx, mask, axis=0)
    # print(mask.shape, fx.shape)
    # data = fx or fy
    # output = []
    # for img in range(mask.shape[0]):
    ...
    #     output.append(valid)
    # return np.ndarray(output, dtype="object")
    # masked = ma.masked_array(fx, fx.shape[2] * [mask])
    # print(masked.shape)
    # print(masked[~masked.mask])
    # return masked[~masked.mask]
    # if type(fx) != None:
    #     for band in range(fx.shape[-1]):
    #         for img in range(band.shape[0]):
    #             print(fx[..., img, band].shape)

    #     # validFx = np.delete(fx.T, mask, axis=2)
    #     # return validFx
    # if type(fy) != None:
    #     validFy = np.delete(
    #         fy, mask, axis=0
...
def generate_dataset(images: list, x_features: list):
    ...
    for image in images:
        ...
        mask = generate_mask(feature_stack_x, feature_stack_y)
        feature_stack_all_x.append(feature_stack_x)
        feature_stack_all_y.append(feature_stack_y)
        mask_stack.append(mask)
    arr_x = np.array(feature_stack_all_x)
    arr_y = np.array(feature_stack_all_y)
    masks = np.array(mask_stack)
    return arr_x, arr_y, masks
